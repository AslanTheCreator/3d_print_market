...
Публичная страница `/sellers/[id]` использует существующие API:

- `userApi.getUserByParams(id)` для данных продавца;
- `productApi.getProducts()` с фильтром `participantId` для списка товаров продавца.

Новые endpoint'ы для этого сценария не вводились. Защищенные данные продавца, платежные реквизиты и настройки доставки не раскрываются через публичный профиль.

Текущий `UserFindModel` не содержит всех визуальных данных, нужных для полноценного публичного профиля. До расширения backend-контракта UI использует временные mock-значения для:

- `description` — краткое публичное описание продавца;
- `totalReviews` — общее количество отзывов продавца.

Чтобы убрать mock-данные, в публичный response `UserFindModel` нужно добавить:

- `description?: string`;
- `totalReviews: number`;

Опционально можно добавить `activeProductsCount: number`, если backend должен показывать точное число активных товаров без вывода его из текущей страницы каталога.
