...
Backend должен гарантировать инвариант: авторизованный пользователь не может
добавить в корзину или заказать товар, продавцом которого является он сам.
Frontend также блокирует этот сценарий, но не является источником безопасности:
клиентскую проверку можно обойти или использовать старую версию приложения.

Проверка обязательна для:

- `POST /basket`;
- `PUT /basket`;
- `GET /order?productId=`;
- `POST /order/BOOKED`;
- целевого `POST /order/checkout`.

Правила:

- ID покупателя определяется только из авторизованной сессии.
- ID продавца загружается backend из актуального товара в БД.
- Нельзя доверять `sellerId` или другим данным о владельце, переданным клиентом.
- Проверка выполняется до создания заказа, резервирования или уменьшения остатка
  и других изменений состояния.
- Если массив `POST /order/BOOKED` содержит собственный товар, весь запрос
  отклоняется без частичного создания заказов.
- Для `POST /order/checkout` вся группа продавца также отклоняется атомарно.
- `DELETE /basket` должен оставаться доступным, чтобы пользователь мог удалить
  ранее сохраненную некорректную позицию.
- Нарушение инварианта является ожидаемой бизнес-ошибкой и не должно приводить
  к HTTP `500`.

Ответ при попытке добавить или заказать собственный товар:
...
Перед выпуском проверки backend должен выполнить одноразовую идемпотентную
миграцию: удалить из корзин позиции, где владелец корзины совпадает с продавцом
товара, и записать в лог количество удаленных записей.
...
- `POST /basket` и `PUT /basket` отклоняют собственный товар с HTTP `422` и
  `code: OWN_PRODUCT_PURCHASE_FORBIDDEN` без изменения корзины.
- `GET /order?productId=`, `POST /order/BOOKED` и `POST /order/checkout`
  отклоняют собственный товар с тем же стабильным кодом.
- Отказ создания заказа не изменяет остаток и не создает заказ полностью или
  частично.
- Миграция удаляет существующие собственные товары из корзин и безопасна при
  повторном запуске.
- Покупка товара другого продавца сохраняет текущее поведение.
- Попытка покупки собственного товара никогда не возвращает HTTP `500`.
